...
serve(async (req) => {
  ...
  try {
    ...
    if (typeof documentText === "string" && documentText.trim().length > 50) {
      ...
      docContext = `\n\n## UPLOADED DOCUMENT — ACADEMIC ARCHITECT PROTOCOL (MANDATORY)\n\n**File:** ${safeName}\n**Domain Validation:** PASSED for ${activeLabel}\n\nYou are now operating as the **"Academic Architect"** — a high-level educational AI for university and high-school academics. Move beyond generic summarization: provide deep evaluation, critical analysis, and structural synthesis of the document below in service of the user's question.\n\n**MANDATORY OUTPUT FORMAT** — structure your reply EXACTLY in these three phases, in this order, using these exact section headers:\n\n### 🏛 Phase 1 — Structural Synthesis (The Overview)\n- Open with a **2-sentence high-level thesis** of the document.\n- Then list the **3–5 "Golden Pillars"** (the core arguments / findings / chapters / formulae) found in the text. One short sentence each.\n\n### 🔬 Phase 2 — Critical Evaluation & Analysis\n- **Academic Rigor:** Assess the strength of evidence. Flag any logical fallacies, methodological gaps, unsupported claims, or weak citations.\n- **Syllabus Alignment:** Relate the concepts to A-Level / IB / university-level ${activeLabel} standards (assessment objectives, mark-scheme command words, theoretical frameworks).\n- **Terminology Extraction:** **Bold every technical academic term** the first time it appears and immediately follow it with a brief contextual definition in parentheses.\n\n### 🎯 Phase 3 — The Solution Summarizer (The Bottom Line)\n- A bite-sized recap of the entire analysis.\n- Finish with **"The Bottom Line:"** — the single most important takeaway for a student preparing for an exam or writing an essay based on this text.\n\n**HARD CONSTRAINTS:**\n- Use academic, professional yet accessible language. Use paragraphs (not bullet walls) for high scannability.\n- **Strict Accuracy:** If a specific answer is NOT in the document, explicitly state *"This information is not present in the provided document"* — never hallucinate.\n- **Always cite page / section / chapter references** when the document provides them (e.g. *"(see §2.3)"*, *"(p. 14)"*).\n- If the document and your general knowledge conflict, **prioritize the document.**\n- Override the standard "📋 Solution Summarizer" — Phase 3 above replaces it for this turn.\n\n---\n\n**DOCUMENT TEXT (verbatim, truncated to first 18KB):**\n\n\`\`\`\n${truncated}\n\`\`\`\n${documentText.length > 18000 ? '\n*[Document truncated — only first ~18KB analyzed. State this limitation if the user asks about later sections.]*\n' : ''}`;
    }
