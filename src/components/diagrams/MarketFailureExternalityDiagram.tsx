...
const MarketFailureExternalityDiagram =  => {
 ...
 ? 'bg-destructive/20 text-destructive border border-destructive/30': 'bg-muted/30 text-muted-foreground hover:bg-muted/50'
 }`}
 >
 Negative Production
 </button>
 ...
 ? 'bg-cambridge-green/20 text-green-400 border border-green-400/30': 'bg-muted/30 text-muted-foreground hover:bg-muted/50'
 }`}
 >
 Positive Production
 </button>
 ...
 ? `${xScale(data.qOptimal)},${yScale(data.pOptimal)} ${xScale(data.qMarket)},${yScale(data.pMarket)} ${xScale(data.qOptimal)},${yScale(data.pMarket - 15)}`: `${xScale(data.qMarket)},${yScale(data.pMarket)} ${xScale(data.qOptimal)},${yScale(data.pOptimal)} ${xScale(data.qMarket)},${yScale(data.pOptimal - 15)}`
 }
