        ...
        <div className="lg:w-1/3 space-y-4">
          <div className={`p-4 rounded-lg border ${gapType === 'recessionary' ? 'bg-red-500/10 border-red-500/50' : 'bg-cyan-500/10 border-cyan-500/50'}`}>
            ...
            <div className="text-sm text-white/80 space-y-2">
              <p>
                ...
                {gapType === 'recessionary' 
                  ? <span>When equilibrium output falls short of full-employment output: <InlineMath math="Y < Y_{fe}" /></span>
                  : <span>When equilibrium output exceeds full-employment output: <InlineMath math="Y > Y_{fe}" /></span>
                }
              </p>
              <p>
                <strong>Indicators:</strong>{' '}
                {gapType === 'recessionary' 
                  ? 'Cyclical unemployment, spare capacity, deflationary pressure'
                  : 'Labour shortages, capacity constraints, inflationary pressure'
                }
              </p>
            </div>
          </div>
          ...
          <div className="p-4 rounded-lg bg-white/5 border border-white/10">
            ...
            <p className="text-xs text-white/70">
              {gapType === 'recessionary' 
                ? 'In the long run, unemployment exerts downward pressure on wages → SRAS shifts right → economy returns to Y* at lower P. However, wage rigidity means this adjustment can be slow and painful.'
                : 'In the long run, excess demand bids up wages → SRAS shifts left → economy returns to Y* at higher P. The inflationary adjustment is typically faster than the deflationary one.'
              }
            </p>
          </div>
          ...
          <div className="p-4 rounded-lg bg-slate-800/50 border border-amber-500/30">
            ...
            <p className="text-xs text-white/70">
              {gapType === 'recessionary' 
                ? 'Keynesian economists argue that waiting for self-correction imposes unnecessary welfare costs. Discretionary fiscal/monetary expansion can close the gap faster.'
                : 'If the inflationary gap is left unchecked, it risks embedding inflation expectations into wage negotiations, creating a wage-price spiral.'
              }
            </p>
          </div>
        </div>
