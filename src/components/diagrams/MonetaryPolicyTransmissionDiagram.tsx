...
const MonetaryPolicyTransmissionDiagram: React.FC<MonetaryPolicyTransmissionDiagramProps> = ({
 ...
 activeStep === step.id 
 ? 'bg-card shadow-lg': 'border-border bg-card/50 hover:border-muted-foreground'
 }`}
 style={{ 
 ...
 ? '↓r → MEC curve unchanged but cost of borrowing falls → More projects where MEC > r → ↑I': '↑r → Cost of borrowing rises above MEC for marginal projects → ↓I'
 }
 </p>
 </div>
 ...
 <p className="text-[10px] text-muted-foreground leading-relaxed">
 {policyType === 'expansionary' 
 ? '↓r → Lower return on saving → Opportunity cost of spending falls → ↑C; Also: cheaper credit for durables': '↑r → Higher return on saving → Opportunity cost of spending rises → ↓C; Also: dearer mortgages → ↓Wealth effect'
 }
 </p>
 </div>
 ...
 <p className="text-[10px] text-muted-foreground leading-relaxed">
 {policyType === 'expansionary' 
 ? '↓r → "Hot money" outflows → Currency depreciates → Exports cheaper, imports dearer → ↑(X-M)': '↑r → "Hot money" inflows → Currency appreciates → Exports dearer, imports cheaper → ↓(X-M)'
 }
 </p>
 </div>
 </div>
 ...
 {policyType === 'expansionary' 
 ? '↓r → ↑I + ↑C + ↑(X-M) → →AD → ↑Y, ↑P, ↑Employment': '↑r → ↓I + ↓C + ↓(X-M) → ←AD → ↓Y, ↓P, ↓Inflation'
 }
 </div>
