...
const SystemStatus = ({ streamState }: { streamState: StreamState }) => {
 ...
 ? 'hsl(0, 84%, 60%)': isProcessing
 ? 'hsl(43, 72%, 53%)': 'hsl(142, 71%, 45%)',
 }}
 />
 </div>
 <span
...
export default function EconomicsChatbot {
 ...
 persona,...(lastUserMsg?.imageUrl ? { image: lastUserMsg.imageUrl }: {}),...(documentText ? { documentText, documentName }: {}),
 }),
 ...
 id: generateId,...(uploadedImage ? { imageUrl: uploadedImage }: {}),
 };
 ...
 ? `⚠️ The image analysis timed out. Try uploading a clearer, smaller photo or crop to the specific diagram area. You can also type out the question text instead.`: `I encountered an issue: ${errorMessage}. Try rephrasing your question or breaking it into simpler parts.`,
 id: generateId,
 isError: true,
 }]);
 } finally {
 ...
 streamChat(userMessages).catch(error => {
 ...
 scrollSnapAlign: 'center',...(isActive ? {
 ...
 willChange: 'transform, background, border-color, box-shadow',
 transform: 'translate3d(0,0,0)',
 transition: 'all 0.1s cubic-bezier(0.22, 1, 0.36, 1)',...(isActive ? {
 ...
 const uploadPromise = supabase.storage.from('persona-uploads').upload(path, file, { contentType: 'application/pdf', upsert: false });
