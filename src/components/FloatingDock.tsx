...
const navigation: NavItem[] = [
  ...
  { title: 'Diagram Bank', href: '/diagrams', icon: Image },
...
const FloatingDock = () => {
  ...
  return (
    <>
      ...
      >
        <div className="floating-dock flex items-center gap-1 px-2 py-2">
          ...
          <Link to="/" className="flex items-center gap-2 px-3 py-1.5 mr-2">
            ...
            <span className="font-serif text-sm font-semibold text-charcoal-gold">MacroMicro</span>
          </Link>

          <div className="w-px h-6 bg-charcoal-gold/20 mx-1" />
          ...
          {navigation.map((item) => (
            ...
            >
              <Link
                to={item.href || '#'}
                className={cn(
                  "dock-item flex items-center gap-2 px-3 py-2 rounded-lg transition-all duration-300",
              ...
              <AnimatePresence>
                {item.levels && activeDropdown === item.title && (
                  <motion.div
                    ...
                    className="absolute top-full left-0 mt-2 dropdown-menu min-w-[280px]"
                  >
                    {item.levels.map((level) => (
                      <div key={level.title} className="mb-3 last:mb-0">
                        <div className="flex items-center gap-2 px-3 py-2 border-b border-charcoal-gold/10">
                          <GraduationCap className="w-3.5 h-3.5 text-charcoal-gold" />
                          <span className="text-xs font-semibold text-charcoal-gold uppercase tracking-wide">
                            {level.title}
                          </span>
                        </div>
                        <div className="py-1">
                          {level.children.map((child) => (
                            <Link
                              ...
                              className={cn(
                                ...
                                isActive(child.href)
                                  ? "text-charcoal-cyan bg-charcoal-cyan/10"
                                  : "text-charcoal-silver hover:text-charcoal-foreground hover:bg-charcoal-light/50"
                              )}
                            >
                              {child.title}
                            </Link>
      ...
      >
        {isMobileMenuOpen ? (
          <X className="w-5 h-5 text-charcoal-foreground" />
        ) : (
          <Menu className="w-5 h-5 text-charcoal-foreground" />
        )}
      </motion.button>
      ...
      >
        <Link to="/" className="floating-dock flex items-center gap-2 px-3 py-2">
          <img src={logoImage} alt="MacroMicro" className="w-6 h-6 object-contain" />
          <span className="font-serif text-sm font-semibold text-charcoal-gold">MacroMicro</span>
        </Link>
      </motion.div>
      ...
      <AnimatePresence>
        {isMobileMenuOpen && (
          ...
          >
            <div 
              className="absolute inset-0 bg-charcoal-base/90 backdrop-blur-md"
                  ...
                  <div key={item.title} className="mb-4">
                    <Link
                      ...
                      className={cn(
                        ...
                        isActive(item.href || '') || isParentActive(item)
                          ? "bg-charcoal-cyan/10 text-charcoal-cyan"
                          : "text-charcoal-silver hover:bg-charcoal-light/50"
                      )}
                    >
                    ...
                    {item.levels && (
                      <div className="ml-6 mt-2 border-l border-charcoal-gold/20 pl-4">
                        {item.levels.map((level) => (
                            ...
                            <div className="mt-1 space-y-1">
                              {level.children.map((child) => (
                                <Link
                                  ...
                                  onClick={() => setIsMobileMenuOpen(false)}
                                  className={cn(
                                    "block py-1.5 text-sm transition-all",
                                    isActive(child.href)
                                      ? "text-charcoal-cyan"
                                      : "text-charcoal-muted hover:text-charcoal-foreground"
                                  )}
                                >
                                  {child.title}
                                </Link>
