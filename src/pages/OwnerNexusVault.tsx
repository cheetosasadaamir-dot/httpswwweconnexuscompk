...
const OwnerNexusVault =  => {
 ...
 setIsLoading(true);
 const { data, error } = await supabase.from('premium_access').select('*').order('created_at', { ascending: false });
 ...
 filter === f
 ? "bg-neon-cyan/20 text-neon-cyan border-neon-cyan/30": "text-muted-foreground bg-card/30 border-border/30 hover:border-neon-cyan/20"
 )}
 >
